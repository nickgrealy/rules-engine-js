...
QUnit.test( "undefined object should log error", function( assert ) {
    assert.strictEqual( engine2.evaluate(), undefined )
});
QUnit.test( "null object should log error", function( assert ) {
    assert.strictEqual( engine2.evaluate(null), undefined )
});
QUnit.test( "blank object should not match anything", function( assert ) {
    assert.strictEqual( engine2.evaluate({}), null )
});
...
QUnit.test( "Strings that match", function( assert ) {
    assert.strictEqual( engine1.evaluate({c1:'string1',c2:'string2'}), 'defaultFunction -> arguments: (out1=aaa,out2=bbb)' )
});
QUnit.test( "Strings that don't match", function( assert ) {
    assert.strictEqual( engine1.evaluate({c1:'stringXX',c2:'stringYY'}), null )
});

// Numbers
QUnit.test( "Numbers that match", function( assert ) {
    assert.strictEqual( engine1.evaluate({c1:123,c2:456}), 'defaultFunction -> arguments: (out1=111,out2=222)' )
});
QUnit.test( "Numbers that don't match", function( assert ) {
    assert.strictEqual( engine1.evaluate({c1:456,c2:123}), null )
});

// Booleans
QUnit.test( "Booleans that match", function( assert ) {
    assert.strictEqual( engine1.evaluate({c1:true,c2:false}), 'defaultFunction -> arguments: (out1=true,out2=false)' )
});
QUnit.test( "Booleans that don't match", function( assert ) {
    assert.strictEqual( engine1.evaluate({c1:false,c2:true}), null )
});

// Objects
QUnit.test( "Objects that match (N.B. compare by reference, not deep comparison!)", function( assert ) {
    assert.strictEqual( engine1.evaluate({c1:obj1,c2:obj2}), 'defaultFunction -> arguments: (out1=[object Object],out2=[object Object])' )
});
QUnit.test( "Objects that don't match", function( assert ) {
    assert.strictEqual( engine1.evaluate({c1:{a:1},c2:{b:2}}), null ) // (not the same object references)
});
...
QUnit.test( "Functions that match", function( assert ) {
    assert.strictEqual( engine1.evaluate({c1:'AA1',c2:'BB2'}), 'defaultFunction -> arguments: (out1=function,out2=function)' )
});
QUnit.test( "Functions that don't match", function( assert ) {
    assert.strictEqual( engine1.evaluate({c1:'FOO',c2:'BAR'}), null )
});

// Undefined
QUnit.test( "Undefined that match", function( assert ) {
    assert.strictEqual( engine1.evaluate({}), 'defaultFunction -> arguments: (out1=undefined,out2=undefined)' )
});
...
QUnit.test( "Null that don't match", function( assert ) {
    assert.strictEqual( engine1.evaluate({c1:null,c2:'bar'}), null )
});
...
QUnit.test( "Wildcards (always match)", function( assert ) {
    assert.strictEqual( engine3.evaluate({c1:'foo',c2:null}), 'defaultFunction -> arguments: (out1=WC1,out2=WC2)' )
});
...
QUnit.test( "No default, no override", function( assert ) {
    assert.strictEqual( engine4.evaluate({scenario:'one'}), undefined )
});
QUnit.test( "No default w/ override", function( assert ) {
    assert.strictEqual( engine4.evaluate({scenario:'two'}), 'two' )
});
QUnit.test( "Default, no override", function( assert ) {
    assert.strictEqual( engine5.evaluate({scenario:'one'}), 'one' )
});
